...
class WebSearchService:
    ...
    async def search(self, query: str, conversation_uuid: str) -> Dict[str, Any]:
        ...
        print([{"Query Number": index + 1, "Query": query} for index, query in enumerate(queries)])

        docs = []

        if len(queries) > 0:
            search_results = await self.search_web(queries, conversation_uuid)
            print('searchResults', [[item["title"] + ' ' + item["url"] for item in r["results"]] for r in search_results])
            ...
            docs = await asyncio.gather(*[
                self._create_document(result, search_result, scraped_content, conversation_uuid)
